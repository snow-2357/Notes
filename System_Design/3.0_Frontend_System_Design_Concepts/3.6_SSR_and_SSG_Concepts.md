...
graph TD
    ...
    C --> D[Browser displays content];
    D --> E[Browser downloads & executes JS];
    E --> F[JS "hydrates" HTML (makes it interactive)];
    F --> G[User interacts with fully interactive page];
```
...
graph TD
    ...
    G --> H[Browser downloads & executes JS (optional hydration)];
```
