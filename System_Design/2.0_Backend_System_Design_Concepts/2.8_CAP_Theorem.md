...
```mermaid
graph TD
    A[CAP Theorem] --> B{Partition (P) Occurs};
    B --> CP[Choose Consistency (CP)];
    B --> AP[Choose Availability (AP)];

    CP --> CP1[Return error from partitioned side];
    CP --> CP2[Block until partition resolves];
    CP --> CP3[Ensures no stale data is read];

    AP --> AP1[Serve requests from both sides];
    AP --> AP2[Risk of stale data reads];
    AP --> AP3[System remains fully operational];
```
